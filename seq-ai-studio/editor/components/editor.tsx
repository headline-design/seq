...
export const Editor: React.FC<EditorProps> = ({ initialMedia, initialClips, onBack }) => {
  ...
  const drawFrameToCanvas = (ctx: CanvasRenderingContext2D, width: number, height: number, time: number) => {
      ...
      const activeClip = timelineClips.find(c => c.trackId.startsWith('v') && time >= c.start && time < c.start + c.duration && c.transition?.type === 'fade-white');
      if (activeClip) {
          const transDuration = activeClip.transition!.duration;
          const progress = (time - activeClip.start) / transDuration;
  ...
  const startExport = async (resolution: '720p' | '1080p', source: 'all' | 'selection') => {
      // Lazy load instance if missing (defensive)
      if (!ffmpegRef.current) {
          ffmpegRef.current = new FFmpeg();
      }

      // Note: We expect the caller (ExportModal) to have called loadFFmpeg().
      // However, we check loaded state here just in case.
      if (!ffmpegLoaded) {
          console.warn("FFmpeg not loaded, attempting lazy load...");
          try {
              await loadFFmpeg();
          } catch(e) {
               alert("Failed to load export engine. Please try again.");
               return;
          }
      }
      ...
      try {
        ...
        const uniqueMediaIds = new Set(timelineClips.filter(c => (c.trackId.startsWith('a') || !c.isAudioDetached)).map(c => c.mediaId));
        ...
        for (const mid of uniqueMediaIds) {
            ...
            if (item && item.url) {
                ...
                } catch (e) { console.warn("Skipping corrupt audio:", e); }
            }
        }
        ...
        timelineClips.forEach(clip => {
            ...
            if (buffer) {
                ...
                if (clip.transition && clip.transition.type !== 'none' && clip.transition.type !== 'wipe-left' && clip.transition.type !== 'wipe-right') {
                ...
                const nextClip = timelineClips.find(c => c.trackId === clip.trackId && Math.abs(c.start - (clip.start + clip.duration)) < 0.05 && c.transition && c.transition.type !== 'none');
                if (nextClip && nextClip.transition && (nextClip.transition.type === 'cross-dissolve' || nextClip.transition.type === 'fade-black')) {
                    const fadeOutDuration = Math.min(nextClip.transition.duration, clipDuration);
                    gainNode.gain.setValueAtTime(baseVolume, offlineCtx.currentTime + startInDest + clipDuration - fadeOutDuration);
        ...
        if (isVertical) { const tmp = targetW; targetW = targetH; targetH = tmp; }
        ...
        while (exportTime < exportEndTime && isMountedRef.current) {
             ...
             const blob: Blob = await new Promise((resolve, reject) => {
                 try { canvasRef.current!.toBlob(b => b ? resolve(b) : reject(new Error("Canvas toBlob failed")), 'image/jpeg', 0.9); }
                 catch (e) { reject(e); }
             });
        ...
        for(let i=0; i<frameCount; i++) {
             try { await ffmpeg.deleteFile(`frame${i.toString().padStart(4, '0')}.jpg`); } catch(e) {}
        }
      ...
      setTimelineClips(prev => {
          ...
          newClips.forEach(newClip => {
             updated = updated.map(c => {
                 if(c.trackId === newClip.trackId && c.start >= newClip.start) { return { ...c, start: c.start + newClip.duration }; }
                 return c;
             });
  ...
  const onToolChange = useCallback((newTool: 'select' | 'razor') => { setTool(newTool); }, []);
        ...
        <div className="flex flex-col gap-6 w-full items-center">
            {['create', 'library', 'transitions', 'inspector', 'settings'].map(v => (
                <div
                    key={v}
                    onClick={() => {
                        if (activeView === v) { setIsPanelOpen(!isPanelOpen); } else { setActiveView(v as any); setIsPanelOpen(true); }
                    }}
         ...
         <div className="flex-1 flex overflow-hidden relative">
            {isPanelOpen && !isCinemaMode && (
                <div className="flex flex-col border-r border-neutral-800 bg-[#09090b] shrink-0 overflow-hidden relative" style={{ width: sidebarWidth }}>
                    {activeView === 'library' && (
                        <ProjectLibrary
                            media={media} selectedId={selectedClipIds[0]} onSelect={(m) => setSelectedClipIds([m.id])} onAddToTimeline={handleAddToTimeline} onImport={handleImport} onRemove={handleRemoveMedia} onClose={() => setIsPanelOpen(false)}
                        />
                    )}
                    ...
                    {activeView === 'inspector' && (
                        <InspectorPanel onClose={() => setIsPanelOpen(false)} selectedClipId={selectedClipIds[0]} clips={timelineClips} mediaMap={mediaMap} tracks={tracks} onUpdateClip={handleClipUpdate} />
                    )}
            ...
            <div className="flex-1 flex flex-col bg-[#09090b] min-w-0 relative">
                <div className="flex-1 w-full bg-[#050505] relative flex items-center justify-center p-6 overflow-hidden min-h-[200px]">
                     ...
                     <div className="relative aspect-video w-full max-h-full shadow-2xl bg-black flex items-center justify-center overflow-hidden" style={{ transform: `scale(${playerZoom})` }}>
                         ...
                         <div ref={whiteOverlayRef} className="absolute inset-0 bg-white pointer-events-none z-20" style={{ opacity: 0 }} />
