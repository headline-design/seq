...
export const Editor: React.FC<EditorProps> = ({ initialMedia, initialClips, onBack }) => {
  ...
  const startExport = async (resolution: "720p" | "1080p", source: "all" | "selection") => {
    ...
    console.log("[v0] Export debug:", {
      ...
      videoRefA: !!videoRefA.current,
      videoRefB: !!videoRefB.current,
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (document.activeElement?.tagName === "INPUT" || document.activeElement?.tagName === "TEXTAREA" || isExporting)
        return
      if (e.code === "Space") {
        e.preventDefault()
        setIsPlaying((p) => !p)
  ...
  const handleCancelExport = () => {
    ...
    exportCancelledRef.current = true // Mark as cancelled
    setIsExporting(false)
            ...
            <div className="flex-1 w-full bg-[#050505] relative flex items-center justify-center p-6 overflow-hidden min-h-[200px]">
              ...
              >
                <video
                  ...
                  crossOrigin="anonymous"
                  onClick={() => !isExporting && setIsPlaying(!isPlaying)}
                />
                <video
                  ref={videoRefB}
