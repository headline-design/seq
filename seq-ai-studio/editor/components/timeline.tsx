...
export const Timeline: React.FC<TimelineProps> = ({
    ...
    const [dragState, setDragState] = useState<{
        ...
        startX: 0,
        initialStates: {},
        minStartDelta: -Infinity,
        maxStartDelta: Infinity
    });

    // Keep refs up to date for event listeners to avoid re-binding
    const clipsRef = useRef(clips); clipsRef.current = clips;
    const zoomLevelRef = useRef(zoomLevel); zoomLevelRef.current = zoomLevel;
    const dragStateRef = useRef(dragState); dragStateRef.current = dragState;
    const snapConfigRef = useRef(snapConfig); snapConfigRef.current = snapConfig;
    const currentTimeRef = useRef(currentTime); currentTimeRef.current = currentTime;
    const isSelectingRef = useRef(isSelecting); isSelectingRef.current = isSelecting;
    const tracksRef = useRef(tracks); tracksRef.current = tracks;
    const toolRef = useRef(tool); toolRef.current = tool;

    // Handlers refs
    const onClipUpdateRef = useRef(onClipUpdate); onClipUpdateRef.current = onClipUpdate;
    const onSelectClipsRef = useRef(onSelectClips); onSelectClipsRef.current = onSelectClips;
    ...
    const onSplitClipRef = useRef(onSplitClip); onSplitClipRef.current = onSplitClip;
    ...
    const handleMouseDownClip = (e: React.MouseEvent, clip: TimelineClip, mode: DragMode) => {
        ...
        const activeClipIds = (isAlreadySelected || !isMultiSelect)
        ...
        let globalMinStartDelta = -Infinity;
        let globalMaxStartDelta = Infinity;
    ...
    const isScrubbingRef = useRef(isScrubbing); isScrubbingRef.current = isScrubbing;
            ...
            <div className="h-10 border-b border-neutral-800 flex items-center justify-between px-4 bg-[#09090b] shrink-0 z-30">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        >
                            {tool === 'razor' ? <ScissorsIcon className="w-3.5 h-3.5" /> : <div className="w-3.5 h-3.5 border-2 border-current rounded-sm"></div>}
                            <span className="text-[10px] uppercase font-bold tracking-wider">{tool === 'razor' ? 'Razor' : 'Select'}</span>
                        </div>
                    ...
                    <div className="relative" onClick={(e) => e.stopPropagation()}>
                        ...
                        {showSnapMenu && (
                            <div className="absolute top-full left-0 mt-1 w-48 bg-[#18181b] border border-neutral-700 rounded-lg shadow-xl py-2 z-50 flex flex-col gap-1">
                                <div className="px-3 py-1 text-[10px] font-bold text-neutral-500 uppercase tracking-wider">Snap Targets</div>
                                {[
                                ...
                                <div className="px-3 py-1 text-[10px] font-bold text-neutral-500 uppercase tracking-wider">Grid Interval</div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2 bg-[#18181b] rounded-md p-1 border border-neutral-800">
                        <button onClick={() => onZoomChange(Math.max(10, zoomLevel - 10))} className="p-1 hover:text-white text-neutral-500">
                        ...
                        <button onClick={() => onZoomChange(Math.min(200, zoomLevel + 10))} className="p-1 hover:text-white text-neutral-500">
                        ...
                        return (
                            <div key={track.id} className={`${trackHeight} border-b border-neutral-800/50 flex flex-col px-3 py-3 gap-2 bg-[#09090b] relative group/header`}>
                                ...
                                <div className="flex-1 bg-neutral-900/30 rounded border border-neutral-800/50 flex flex-col items-center justify-center overflow-hidden relative">
                                    {isAudio ? (
                                        <div className={`w-full h-full flex flex-col items-center justify-center px-2 gap-1 transition-opacity ${track.isMuted ? 'opacity-30' : 'opacity-100'}`}>
                ...
                >
                        ...
                        <div className="relative w-full h-full">
                            {[...Array(Math.ceil(duration + 10))].map((_, sec) => (
                                <div key={sec} className="absolute bottom-0 h-full flex flex-col justify-end pointer-events-none" style={{ left: `${(sec * zoomLevel)}px` }}>
                                    ...
                                    {snapConfig.enabled && snapConfig.toGrid && (
                                    ...
                                    {(!snapConfig.enabled || !snapConfig.toGrid || snapConfig.gridInterval === 1) && (
                                        <div className="absolute bottom-0 border-l border-neutral-700 h-2" style={{ left: `${(zoomLevel * 0.5)}px` }}></div>
                                    )}
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="min-w-full relative" style={{ width: `${Math.max(duration * zoomLevel + 500, 2000)}px` }}>
                        ...
                        {tracks.map(track => {
                            ...
                            return (
                                <div key={track.id} className={`${trackHeight} border-b border-neutral-800/30 relative group/track bg-[#0c0c0e]`}>
                                    <div className="absolute inset-0 bg-[linear-gradient(90deg,transparent_99%,#1f1f22_100%)] opacity-10 pointer-events-none" style={{ backgroundSize: `${zoomLevel}px 100%` }}></div>
                                    ...
                                    {trackClips.map(clip => {
                                        ...
                                        const selectedClass = isSelected
                                            ? (isAudio ? 'bg-emerald-900/60 border-emerald-400 z-20 ring-1 ring-emerald-400' : 'bg-[#1e1e24] border-[#6366f1] z-20 ring-1 ring-[#6366f1]')
